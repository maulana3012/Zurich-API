...
//parsing-data
        ...
        if($number_of_rows>0){
            ...
            $countupdated=0;
	        $link_zurich="http://$hostname/$riderect/?uid=";
            if(count($result)>0){
			    ...
			    'LINK_CREATED' => date('yymd'),
			    'CYCLE_DATE' => $val['CYCLE_DATE']
                            );
                    var_dump($data);
                    $api_client = ApiClient::factory($hostname, 'zurpro8', $username,$password);
...
//export-excel
	...
	if ($query->num_rows > 0) {
	    ...
	    $filename = $localdir."Datafeed-export-eov".date('Ymd').".csv";
